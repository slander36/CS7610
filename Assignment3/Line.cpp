...
void Line::draw(Colors::rgba rgbas, Colors::rgba rgbae) {
	/* Prepare OpenGL */

	/* Set Line Thickness */
	glPointSize(lineThickness);

	/* Set Variables for Goraud Shading */
	float cRed = rgbas.red;
	float cGreen = rgbas.green;
	float cBlue = rgbas.blue;
	float cAlpha = rgbas.alpha;
	float uRed = (rgbas.red - rgbae.red) / (xe - xs);
	float uGreen = (rgbas.green - rgbae.green) / (xe - xs);
	float uBlue = (rgbas.blue - rgbae.blue) / (xe - xs);
	float uAlpha = (rgbas.alpha - rgbae.alpha) / (xe - xs);
	...
	int steps = (steep ? deltaY : deltaX );
	for( int i = 0 ; i < steps ; i++ ) {
		// Goraud Shading Stuff		
		/* Set Draw Color */
		// Set the current color
		glColor4f(cRed, cGreen, cBlue, cAlpha);
		// Update the color
		cRed = cRed - uRed;
		cGreen = cGreen - uGreen;
		cBlue = cBlue - uBlue;
